...
class ADB:
    ...
    def screenshot(self, filename: str = None) -> str:
        """Captura screenshot corrigido"""
        if not filename:
            filename = f"screenshot_{int(time.time())}.png"
        
        try:
            # Método 1: Usar screencap direto
            success, _ = self._run_cmd(['shell', 'screencap', '-p', f'/sdcard/{filename}'])
            if success:
                # Baixar arquivo
                success2, _ = self._run_cmd(['pull', f'/sdcard/{filename}', filename])
                if success2:
                    # Limpar arquivo do dispositivo
                    self._run_cmd(['shell', 'rm', f'/sdcard/{filename}'])
                    logger.info(f"📸 Screenshot salvo: {filename}")
                    return filename
            
            # Método 2: Usar exec-out (fallback)
            ...
            logger.error("❌ Falha em ambos os métodos de screenshot")
            return None
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if not success:
                logger.error("❌ ADB não está funcionando")
                return False
            
            # Verificar se há dispositivos
            lines = output.split('\n')
            devices = [line for line in lines if 'device' in line and 'devices attached' not in line]
            
            if not devices:
                logger.error("❌ Nenhum dispositivo conectado")
                logger.info("💡 Conecte o celular e ative 'Depuração USB'")
                return False
            
            # Testar comando simples
            success, _ = self._run_cmd(['shell', 'echo', 'test'])
            if not success:
                logger.error("❌ Não consegue executar comandos no dispositivo")
                return False
            
            logger.info(f"✅ ADB conectado: {len(devices)} dispositivo(s)")
            return True
            
        except Exception as e:
            logger.error(f"❌ Erro ao testar ADB: {e}")
            return False
...
class InstagramBot:
    ...
    def follow_user(self, username: str, profile_link: str) -> bool:
        ...
        try:
            ...
            if not browser_screenshot:
                logger.warning("⚠️ Screenshot falhou, continuando sem OCR...")
                # Usar coordenadas fixas como fallback
                self.adb.tap(self.adb.screen_width // 2, int(self.adb.screen_height * 0.75))
                time.sleep(5)
                
                # Tentar follow direto
                self.adb.tap(self.adb.screen_width // 2, int(self.adb.screen_height * 0.85))
                time.sleep(2)
                
                self.db.mark_followed(username)
                logger.info(f"✅ Seguiu (fallback): {username}")
                return True
            ...
            if not instagram_btn_coords:
                logger.warning("⚠️ OCR falhou, usando coordenadas aproximadas...")
                # Fallback para coordenadas baseadas na tela
                instagram_btn_coords = (self.adb.screen_width // 2, int(self.adb.screen_height * 0.75))
            ...
            # 7. Encontrar botão "Seguir"
            follow_btn_coords = None
            if app_screenshot:
            ...
            if not follow_btn_coords:
                logger.warning("⚠️ Botão 'Seguir' não encontrado via OCR, usando posição aproximada...")
                # Posição típica do botão seguir
                follow_btn_coords = (self.adb.screen_width // 2, int(self.adb.screen_height * 0.85))
            
            # 8. Verificar cor do botão se temos screenshot
            if app_screenshot:
    ...
    def run_follow_session(self):
        ...
        # Testar conexão ADB primeiro
        if not self.adb.test_connection():
            logger.error("❌ Problema com ADB - sessão cancelada")
            return
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            print("🔧 Testando conexão ADB...")
            if bot.adb.test_connection():
                print("✅ ADB funcionando corretamente!")
                # Testar screenshot
                print("📸 Testando screenshot...")
                filename = bot.adb.screenshot("test_screenshot.png")
                if filename:
                    print(f"✅ Screenshot funcionando: {filename}")
                else:
                    print("❌ Screenshot com problemas")
            else:
                print("❌ Problema com ADB")
        elif choice == '9':
            break
        else:
